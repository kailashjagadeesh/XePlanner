                                                 ...
                                                 const JointConfig &q_goal) const
{
    ...
    // Hash agent_id
    h ^= ih(agent_id) + 0x9e3779b9 + (h << 6) + (h >> 2);
    
    // Hash start config
    for (double q : q_start)
    {
...
bool XeCBSPlanner::queryExperience(int agent_id,
                                    ...
                                    AgentPath &out_path)
{
    // Compute key based only on agent, start, goal
    std::size_t hash_key = computeExperienceHash(agent_id, q_start, q_goal);
    
    // Get range of all paths that match this start/goal
    auto range = experience_db_.equal_range(hash_key);
    
    // Iterate through all candidates
    std::vector<const AgentPath*> valid_paths;

    for (auto it = range.first; it != range.second; ++it)
    {
        const ExperienceEntry& entry = it->second;
        const AgentPath &cached_path = entry.path;

        // Validate: Check if path starts and ends correctly (hash collision check)
        if (cached_path.empty()) continue;
        
        const double EPSILON = 0.1;
        bool start_matches = true;
        bool goal_matches = true;
        
        // Check start configuration
        if (cached_path[0].size() == q_start.size())
        {
            for (size_t i = 0; i < q_start.size(); ++i)
            {
                if (std::fabs(cached_path[0][i] - q_start[i]) > EPSILON)
                {
                    start_matches = false;
                    break;
                }
        ...
        if (cached_path.back().size() == q_goal.size())
        {
            for (size_t i = 0; i < q_goal.size(); ++i)
            {
                if (std::fabs(cached_path.back()[i] - q_goal[i]) > EPSILON)
                {
                    goal_matches = false;
                    break;
                }
        ...
        if (!start_matches || !goal_matches) continue;
        
        // Validate: Check if path violates constraints
        bool violates_constraints = false;
        for (size_t t = 0; t < cached_path.size(); ++t)
        {
            if (violatesVertexConstraints(agent_id, cached_path[t], static_cast<int>(t), constraints))
            {
        ...
        // Return cached path only if it's valid
        if (!violates_constraints)
        {
            valid_paths.push_back(&cached_path);
        }
    }

    if (!valid_paths.empty())
    {
        // Sort by length (cost) to prioritize shorter paths
        std::sort(valid_paths.begin(), valid_paths.end(), 
            [](const AgentPath* a, const AgentPath* b) {
                return a->size() < b->size();
            });

        // Check for bounded suboptimality

        out_path = *valid_paths[0];
        experience_hits_++;
        return true;
    }
                                    ...
                                    const AgentPath &path)
{
    ...
    // Check for exact duplicate path to avoid redundancy
    auto range = experience_db_.equal_range(hash_key);
    for (auto it = range.first; it != range.second; ++it) {
    ...
    experience_db_.insert({hash_key, entry});
}
                              ...
                              AgentPath &out_path)
{
    ...
    if (queryExperience(agent_id, q_start, q_goal, all_constraints, out_path))
    {
        ...
        std::cout << "Found path for agent " << agent_id << " (from cache)" << std::endl;
        return true;
    }
...
std::vector<Node *> XeCBSPlanner::plan(const std::vector<std::vector<double>> &start_poses, const std::vector<std::vector<double>> &goal_poses)
{
    ...
    root->depth = 0;
    // std::vector<Conflict> root_conflicts;
    // findAllConflicts(root->paths, root_conflicts);
    // root->num_conflicts = root_conflicts.size();
        ...
        if (focal.empty())
        {
            ...
            if (!has_conflict)
            {
                std::cout << "[XeCBS] Found conflict-free solution with cost " << node->cost 
                         << " (suboptimality: " << suboptimal_factor_ << ") after " << iter << " iterations.\n";
