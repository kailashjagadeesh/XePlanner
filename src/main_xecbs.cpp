...
int main()
{
    ...
    if (plan.empty())
    {
        std::cerr << "[MAIN] XeCBS failed to find a plan.\n";
        plan.push_back(new Node(start_poses, 0.0));
    }
    else
    {
        std::cout << "[MAIN] XeCBS returned plan with " << plan.size() << " waypoints\n";
    }
